<x-app-layout>
  ...
  <main class="relative container mx-auto bg-white px-4">
    ...
    <article class="max-w-prose mx-auto py-8">
      <h1 class="text-2xl font-bold">{{ $actualite->titre }}</h1>
      <h2 class="mt-2 text-sm text-gray-500 "> {{ $actualite->created_at->format('d M Y')}}
      </h2>

      <p class="mt-6 text-justify">{{ $actualite->description }}</p>
    </article>
  </main>
</x-app-layout>